...
// Sort probabilities if available
...
// Fetch recommendation based on disease
...
// Set flags for pending status and low confidence
...
<script>
  ...
  if (isPending) {
    ...
    .then(d => {
      ...
      if (d.status === "success") {
          // Success: smoothly fade in results without reloading
          updateCircle(100);
          setTimeout(() => {
            ...
            document.getElementById("recomArea").classList.remove("hidden-area");
            ...
            document.getElementById("recomText").innerHTML = d.recommendation ? d.recommendation.replace(/\n/g, "<br>") : "";
          ...
          // AI Error: Turn circle red and show error message
          circleProg.style.background = '#fee2e2'; 
          circlePerc.innerText = 'Fail';
          circlePerc.style.color = '#dc2626';
          document.querySelector('.circle-text').innerText = 'Error';

          let sb = document.getElementById("statusBox");
          sb.className = "status-box status-error";
          sb.innerHTML = "❌ <b>Analysis Failed:</b> " + d.message;

          if(d.debug) console.error("Python Output:", d.debug);
      }
    }).catch(e => {
        clearInterval(timer);
        if(circleProg) circleProg.style.background = '#fee2e2';
        if(circlePerc) {
        ...
        if(sb) {
            sb.className = "status-box status-error";
            sb.textContent = "❌ Network/Server Error occurred.";
        }
    });
  } else {
      // If page is already completed on load
